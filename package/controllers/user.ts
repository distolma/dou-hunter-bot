...
export const createUser = ({ from: { id, first_name, last_name, username } }: Message) =>
  (new User({ tel_id: id, first_name, last_name, username } as IUser)).save();

export const updateUser = ({ from: { id, first_name, last_name, username } }: Message) =>
  User.findOneAndUpdate({ tel_id: id }, { first_name, last_name, username }).exec();
